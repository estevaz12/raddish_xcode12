...
class ViewModel: ObservableObject {
  ...
  @Published var recipesExpSoon = [Recipe]()
  ...
  var recipesExpSoonNames = [String]()
...
func populateRecipes() {
  ...
  func populateRecipesExpSoon() {
      for recipe in recipesExpSoonNames {
          createRecipe(recipe:recipe)
      }
  ...
  func createRecipeExpSoon(recipe:String){
      //var recipeInstance: Recipe  = Recipe()
      self.recipesExpSoon = [ ]
            ...
            let task = URLSession.shared.dataTask(with: URL(string: url)!)
              ...
              print(recipeInstance.strInstructions)
              self.recipesExpSoon.append(recipeInstance)
            }
            task.resume()
        }
  
  func fetchRecipesExpSoon() {
    self.recipesExpSoonNames = []
    let ingredients = fetchItemsExpiringSoon()
    
    for ing in ingredients {
      var meal = retrieveMealswithIng(ingredient: ing.displayName())
      self.recipesExpSoon.append(meal.strMeal)
    }
  }
